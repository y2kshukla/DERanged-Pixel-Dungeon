...
public interface AlchemyWeapon {
    ...
    static String hintString(ArrayList<Class<?extends Item>> recipe) {
        if (recipe.get(1).isAssignableFrom(UpgradeDust.class)) {
            return Messages.get(Item.class, "discover_hint_alchemy_one", Reflection.newInstance(recipe.get(0)).trueName());
        } else {
            return Messages.get(Item.class, "discover_hint_alchemy_two", Reflection.newInstance(recipe.get(0)).trueName(), Reflection.newInstance(recipe.get(1)).trueName());
        }
    }
